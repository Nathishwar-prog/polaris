...
export const createScrapeUrlsTool = () => {
  return tool({
    ...
    execute: async (params: { urls: string[] }) => {
      // In strict OpenAI tool calling, z.string() in parameters might come as raw string or parsed.
      // ai sdk parameters are verified against 'parameters' schema.
      // paramsSchema here is a second layer, we can relax it or cast.
